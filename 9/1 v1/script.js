...
function exerciseTypeOne(Num) {
	...
	let outputText ='<strong>' + (Num+1) + '.</strong> В одной из кодировок каждый символ кодируется <strong>' + kolvoBit + '</strong> битами.<br>Ученик записал текст: <br><br>"Города '
	...
	outputText += '<br>После этого ученик убрал из текста один из городов, а также ставшие лишними запятую и пробел. После этого размер предложения оказался на <strong>'
...
function exerciseTypeTwo(Num) {
	const bits = [8,16,32];
	const kolvoBit = bits[Math.floor(Math.random() * 3)];
	let outputText ='<strong>' + (Num+1) + '.</strong> В одной из кодировок каждый символ кодируется <strong>' + kolvoBit + '</strong> битами. Определите размер в байтах следующего предложения в данной кодировке, не учитывая кавычки: ';
	const Sentence = sentences[Math.floor(Math.random() * sentences.length)];
	outputText += '"<strong>' + Sentence + '</strong>"';
